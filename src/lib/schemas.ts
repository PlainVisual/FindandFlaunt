...
export const SearchFormSchema = z.object({
  ...
  searchResultsHtml: z.string().min(100, { message: "Search results HTML must be provided (minimum 100 characters)." })
    .refine(val => val.toLowerCase().includes("<html") && val.toLowerCase().includes("</html>"), {
      message: "Please provide valid HTML content that includes <html> tags."
    }),
});
